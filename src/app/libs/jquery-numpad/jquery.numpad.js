...
(function($) {
    ...
    return this.each(function() {
      ...
      if ($('#' + id).length == 0) {
        ...
        table.append(
          ...
          .append($(options.cellTpl).append($(options.buttonFunctionTpl).html(options.textDelete).addClass('del btn-danger').click(function() {
            nmpd.setValue(nmpd.getValue().toString().substring(0, nmpd.getValue().toString().length - 1));
          })))
          .append($(options.cellTpl).append($(options.buttonFunctionTpl).html(options.textClear).addClass('clear btn-default').click(function() {
            nmpd.setValue('');
          })))
        );
  ...
  $.fn.numpad.defaults = {
    ...
    rowTpl: '<tr></tr>',
    cellTpl: '<td></td>',
    buttonNumberTpl: '<button type="button" class="btn"></button>',
